...
const { create: createShuntingYard, operator: createOperator, SENTINEL } = require('./shunting-yard');
const tokenStream = require('./token-stream');
const builder = require('./node-builder');

module.exports = parseFormula;
module.exports.createBinaryOperator = createBinaryOperator;
...
function parseOperandExpression(stream, shuntingYard) {
  ...
  } else if (stream.nextIsFunctionArgumentSeparator()) {
    shuntingYard.operands.push(builder.blank());
  }
}

function parseFunctionCall(stream, shuntingYard) {
  const token = stream.getNext()
  const name = token.value;
  const attributes = token.attributes;
  stream.consume(); // consume start of function call

  const args = parseFunctionArgList(stream, shuntingYard);
  shuntingYard.operands.push(builder.functionCall(name, attributes, args));
